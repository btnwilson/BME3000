...
def plot_data(signal, fs,title='Title', units='A.U.'):
...
def plot_filtered_data(signal, impulse_response, fs, title = 'Title', units = 'A.U.'):
    filtered_signal = np.convolve(signal, impulse_response, mode = 'same')
    plot_data(filtered_signal, fs, title = title, units = units )
    return filtered_signal

def find_beats(signal, fs):
    flipped_signal = signal * -1
    beats, info = scipy.signal.find_peaks(flipped_signal, height = 100)
    time = np.arange(0,len(signal)/fs, 1/fs)
...
def plot_frequency_bands(signal, fs, low_fc_range, high_fc_range, title = 'Title', units = 'A.U'):
